    ...
    return (
      <div ref={timerRef} className="fixed top-0 left-0 z-[100] flex flex-col items-center" style={{ touchAction: 'none' }}>
              ...
              {mode === 'timer' && (
                 <div className="grid grid-cols-3 gap-2">
                    {[30, 60, 90, 120, 180, 240].map(secs => (
                       <button key={secs} onClick={() => setTimerPreset(secs)} className="px-1 py-1.5 bg-slate-800 hover:bg-slate-700 rounded text-[10px] font-bold text-slate-300 border border-slate-700 hover:border-accent-500 transition">
        ...
        <div 
            ...
            className={`w-14 h-14 rounded-full bg-slate-800 border-2 ${isActive && mode === 'timer' && seconds <= 10 ? 'border-red-500 animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.5)]' : isActive ? 'border-accent-500 shadow-[0_0_15px_rgb(var(--accent-500)/0.3)]' : 'border-slate-600'} flex flex-col items-center justify-center shadow-xl cursor-grab active:cursor-grabbing backdrop-blur-md select-none`}
        >
            <Timer size={18} className={isActive && mode === 'timer' && seconds <= 10 ? 'text-red-500' : isActive ? 'text-accent-500' : 'text-slate-400'} style={{ pointerEvents: 'none' }} />
