...
export default function ReverseCalculator({ barWeight, barUnit }) {
    ...
    return (
      <div className="space-y-4 animate-fade-in flex flex-col h-full pb-20 md:grid md:grid-cols-2 md:gap-6 md:space-y-0">
        ...
        <div className="space-y-6">
           <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800">
              ...
              <div className="grid grid-cols-4 gap-2 mb-2">{standardKg.map(w => {
                const cfg = PLATE_CONFIG.kg[w];
                return (
                  <button
                    key={`kg-${w}`}
                    onClick={() => addPlate(w, 'kg')}
                    style={{ backgroundColor: cfg?.fill, borderColor: cfg?.stroke, color: cfg?.text }}
                    className="h-12 rounded-lg font-bold shadow-sm active:scale-95 border-b-4 flex items-center justify-center"
                  >{w}</button>
                );
              })}</div>
              <h5 className="text-[10px] text-slate-500 font-bold uppercase mb-2 mt-3">Micro</h5>
              <div className="grid grid-cols-6 gap-2">{fractionalKg.map(w => {
                const cfg = PLATE_CONFIG.kg[w];
                return (
                  <button
                    key={`kg-${w}`}
                    onClick={() => addPlate(w, 'kg')}
                    style={{ backgroundColor: cfg?.fill, borderColor: cfg?.stroke, color: cfg?.text }}
                    className="h-10 text-xs rounded-lg font-bold shadow-sm active:scale-95 border-b-2 flex items-center justify-center"
                  >{w}</button>
                );
              })}</div>
           </div>
           <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800">
              ...
              <div className="grid grid-cols-4 gap-2 mb-2">{standardLb.map(w => {
                const cfg = PLATE_CONFIG.lb[w];
                return (
                  <button
                    key={`lb-${w}`}
                    onClick={() => addPlate(w, 'lb')}
                    style={{ backgroundColor: cfg?.fill, borderColor: cfg?.stroke, color: cfg?.text }}
                    className="h-12 rounded-lg font-bold shadow-sm active:scale-95 border-b-4 flex items-center justify-center"
                  >{w}</button>
                );
              ...
              <h5 className="text-[10px] text-slate-500 font-bold uppercase mb-2 mt-3">Micro</h5>
              <div className="grid grid-cols-6 gap-2">{fractionalLb.map(w => {
                const cfg = PLATE_CONFIG.lb[w];
                return (
                  <button
                    key={`lb-${w}`}
